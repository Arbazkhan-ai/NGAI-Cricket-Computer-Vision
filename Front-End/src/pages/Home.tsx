import { Play, Activity, Target, Shield, AlertCircle, Info, Camera, Upload } from 'lucide-react';
                ...
                tempCanvas.toBlob(async (blob) => {
                    ...
                    try {
                        ...
                        if (response.data && response.data.length > 0) {
                            ...
                            // Draw boxes and poses
                            response.data.forEach(det => {
                                // Draw Box - DISABLED (User Req: "do not show grid")
                                /*
                                if (det.xyxy) {
                                    ...
                                    ctx.setLineDash([]);
                                    const shotName = det.class_name || 'Unknown Shot';
                                    ctx.fillStyle = '#10b981';
                                    ctx.font = 'bold 20px sans-serif';
                                    ctx.fillText(`${shotName} (${(det.conf * 100).toFixed(0)}%)`, x1, y1 - 10);
                                }
                                */

                                // Draw Keypoints (Pose) - Keeping pose for now as 'grid' usually refers to boxes,
                                // but if "clean" view is wanted, user might want this gone too.
                                // I will hide pose as well to be safe for "do not show grid/overlay".
                                /*
                                if (det.keypoints) {
