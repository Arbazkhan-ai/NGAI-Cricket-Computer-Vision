...
def analyze_ball(track):
    ...
    bounce = any(y[i] > y[i - 1] and y[i] > y[i + 1] for i in range(2, len(y) - 2))

    swing_amount   = abs(x[-1] - x[0])
    spin           = sum(abs(x[i] - 2 * x[i - 1] + x[i - 2]) for i in range(2, len(x)))
    dist           = sum(np.linalg.norm(np.array(track[i]) - np.array(track[i - 1]))
